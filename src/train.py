...
class TrainModel(object):
    ...
    def train_step(self):
        ...
        cum_loss = 0
        self.model.train()
        for inputs, masks, target in tqdm(self.train_loader, ascii=True, desc='train'):
            inputs, masks, target = inputs.to(device), masks.to(device), target.to(device)
    ...
    def main(self):
        ...
        indices = split_list(indices, data_part)
        np.random.shuffle(indices)

        for part in range(data_part):
            """ Data parts """
            num_fold = 5
            kf = KFold(n_splits=num_fold, random_state=42, shuffle=True)
            train_idx = []
            valid_idx = []
            for t, v in kf.split(indices[part]):
                train_idx.append(indices[part, t])
                valid_idx.append(indices[part, v])
                ...
                for epoch in range(args.epoch):
                    ...
                    if (epoch + 1) % scheduler_step == 0:
                        torch.save(best_param, args.save_weight + args.weight_name +
                                   'f' + str(part) + str(num_snapshot) + '.pth')
...
parser.add_argument('--snapshot', default=30, type=int, help='Number of snapshots per fold')
