...
class Train_Model(object):
    ...
    def train(self):
        ...
        for (image, label) in tqdm(self.train_loader):
            ...
            all_outputs = torch.zeros((self.batch_size, 4, 1, 1)).float().cuda()
            with torch.set_grad_enabled(True):
                for index_crop in range(num_crop):
                    im = image[:, index_crop, :, :, :]
                    ...
                    for batch in range(batch_size):
                        for ch in range(4):
                            if outputs[batch, ch, :, :] > all_outputs[batch, ch, :, :]:
                                all_outputs[batch, ch, :, :] = outputs[batch, ch, :, :]
                outputs = torch.squeeze(all_outputs)
    ...
    def main(self):
        ...
        self.train_loader = torch.utils.data.DataLoader(train_set, batch_size=self.batch_size, shuffle=False)
